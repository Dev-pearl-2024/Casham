...
const Registration = (props) => {
    ...
    return (
        <SafeAreaView style={[stylesheet.container, { justifyContent: 'space-between' }]}>
            <View style={{
                top: 45, left: 35,
            ...
            <ScrollView style={{ width: '90%', flex: 1, marginTop: 80, }} showsVerticalScrollIndicator={false}>
                <View style={{ alignSelf: 'flex-start' }}>
                    <Image source={require('../../../Elements/Images/logo.png')} style={{ width: 100, height: 33.54, marginBottom: 20 }} />
                    <Text style={[stylesheet.title, { alignSelf: 'flex-start' }]}>
                        Join <Text style={[stylesheet.title, { color: Colors.primary }]}>Casham </Text>
                    ...
                    {role === 'merchant' ? (
                        <View style={{ marginTop: 30, alignSelf: 'center', marginBottom: 100 }}>
                            <MobileInput marginTop={10} value={mobile} onChangeText={(selected, text) => {
                                setMobile(text.text);
                                setSelectedCode(selected.selectedCode)
                            }} />
                            <CustomTextInput icon={"person"} placeholder={"Full Name"} marginTop={10} value={name} onChangeText={setName} />
                            <CustomTextInput icon={"business"} placeholder={"Business Name"} marginTop={10} value={businessName} onChangeText={setBusinessName} />
                            <CustomTextInput icon={"email"} placeholder={"Email"} marginTop={10} value={email} onChangeText={setEmail} />
                            <CustomTextInput icon={"business-center"} placeholder={"Business Type (Optional)"} marginTop={10} value={businessType} onChangeText={setBusinessType} />
                        </View>
                    ) : (
                        <View style={{ marginTop: 30, alignSelf: 'center' }}>
                            <CustomTextInput icon={"person"} placeholder={"Full Name"} value={name} onChangeText={setName} />
                            <MobileInput marginTop={10} value={mobile} onChangeText={(mobile, text) => {
                                console.log(text);
            ...
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
                ...
                <Text style={{ marginTop: 10, fontSize: 14 }}>By clicking on "Sign Up" you agree to our</Text>
                <TouchableOpacity onPress={() => props.navigation.navigate("PrivacyPolicy")}>
                    <Text style={{ fontWeight: 'bold', color: Colors.primary, fontSize: 14 }}>Terms & Conditions</Text>
                </TouchableOpacity>
            </View>
            <CustomSnackbar label={"Okay"} onDismissSnackbar={() => setSnackVisible(false)} title={snackMessage} visible={snackVisible} />
