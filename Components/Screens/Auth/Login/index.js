...
import { Text, View, Image, ScrollView, Dimensions, Linking, BackHandler } from "react-native";
...
const Login = (props) => {
    ...
    const RequestLocation = async () => {
        await Location.requestForegroundPermissionsAsync().then((rs) => {
            if (!rs.granted) {
                setVisibleDialog(true)
            } else {
                setVisibleDialog(false)
                Location.getLastKnownPositionAsync().then((rs) => {
    ...
    return (
        <SafeAreaView style={stylesheet.container}>
            <View style={{
                width: '100%', marginVertical: 10, paddingHorizontal: 25,
                marginTop: 50
            }}>
                <BackButton props={props} />
            </View>
            <ScrollView style={{}} showsVerticalScrollIndicator={false} contentContainerStyle={{}}>
                <View style={{ width: '90%', alignItems: 'center', marginTop: 20, flex: 1, justifyContent: 'space-between', height: Dimensions.get('window').height / 1.6 }}>
                    <View style={{ alignSelf: 'flex-start' }}>
                        <Image source={require('../../../Elements/Images/logo.png')} style={{ width: 100, height: 33.54, marginBottom: 20 }} />
                        <Text style={[stylesheet.title, { alignSelf: 'flex-start', color: Colors.text }]}>
                            Welcome Back to <Text style={[stylesheet.title, { color: Colors.primary }]}>Casham!</Text>
                        </Text>
                ...
                <View style={{ alignItems: 'center', gap: 20, marginBottom: 20, flex: 1 }}>
                    <CustomButton label={"Send OTP"} onPress={CheckUserExist} />
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Text style={{ fontSize: 14, color: 'gray' }}>Don't have an account?</Text>
                        <TouchableOpacity onPress={() => props.navigation.replace("Registration", { role: 'user' })}>
                            <Text style={{ fontSize: 14, color: Colors.secondary, fontWeight: 'bold' }}> Sign Up</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                <CustomSnackbar visible={snackVisible} title={snackMessage} onDismissSnackbar={onDismissSnackbar} />
                <Loader visible={loading} />
            </ScrollView>
            <CustomDialog visible={visibleDialog}
                onDismiss={() => {
                ...
                }} dialogMessage={"Location permission not granted, please provide acccess to location services to continue"}
                settings />
        </SafeAreaView>
    );
};
