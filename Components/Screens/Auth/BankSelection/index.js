...
const BankScreen = (props) => {
    ...
    const GetAllBanks = async () => {
        ...
        await axios.get("https://casham.webhop.me/api/banks").then((rs) => {
    ...
    const [modalHeight, setModalHeight] = useState(Dimensions.get('window').height / 1.5)
    ...
    const handleBankDetails = async () => {
        ...
        if (accountNumber && BankSelection.code && BankSelection.name) {
            if (accountNumber.length > 16 || accountNumber.length < 15) {
                ...
                await axios.post(baseURL + "Creates", formData,
                    {
                        headers: {
    ...
    return (
        <ScrollView>
            ...
            <Text style={{
                fontWeight: 'bold',
                fontSize: 22, marginStart: 20, marginTop: 30,
                marginBottom: 20
            }}>All Banks</Text>
            {
                    ...
                    return (
                        <TouchableOpacity key={index}
                            style={{
                                ...
                                setDialogMessage(`Selected bank is ${item.name}, Proceed with selection?`)
                            }}>
                            <MaterialCommunityIcons name="bank-outline" size={30}
                                style={{
                                    padding: 5, borderWidth: 0.8, borderColor: 'lightgray',
                                    borderRadius: 5
                                }} />
                            <Text style={{
                                flex: 0.95,
            ...
            <CustomDialog visible={dialogVisible} dialogMessage={dialogMessage}
                onDismiss={() => { setDialogVisible(false) }}
                button1={"Continue"} onPress={() => {
                    setDialogVisible(false)
                    setAccountDetailVisible(true)
                }} />
            <Portal>
                <Modal visible={accountDetailVisible}
                    onDismiss={() => { setAccountDetailVisible(false) }}
                    style={{
                    ...
                    <View style={{
                        ...
                        <Text
                            style={{
                                ...
                                marginRight: 20
                            }}>Please provide account number for {BankSelection.name}</Text>
