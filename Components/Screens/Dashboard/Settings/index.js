...
import { Appearance, Dimensions, Image, Touchable, TouchableOpacity, View } from "react-native"
...
export default Settings = ({ props }) => {
    ...
    const GetUserDetails = async () => {
        ...
        await axios.get(`${baseURL}userdetails`, {
            params: {
                token: device_token,
            },
            headers: {
                    ...
                    .then(async (rs) => {
                        if (!rs.canceled) {
                            ...
                            try {
                                ...
                                const response = await axios.post(`${baseURL}updates`, formData, {
                                    headers: {
                                ...
                                if (response.status == 202) {
                                    ...
                                    setLoading(false)
                                    GetUserDetails()
                                    console.error('Failed to update profile photo:', response.data);
                                }
                            } catch (error) {
                                GetUserDetails()
                                setLoading(false)
    ...
    const handleDeregister = async () => {
        ...
        await axios.delete(baseURL + "deleteuser", {
            headers: {
                'Authorization': `Bearer ${api_token}`,
        ...
        <SafeAreaView style={[stylesheet.container, {
                    ...
                    <Menu
                        ...
                        anchor={
                            <TouchableScale onPress={openMenu}>
                                <Image source={{
                                    uri:
                                        data?.photo ? data.photo : 'https://thumbs.dreamstime.com/b/default-avatar-profile-icon-vector-social-media-user-image-182145777.jpg'
                                }}
                                    style={{
                        ...
                        <Menu.Item leadingIcon="camera" onPress={() => {
                            TakePhoto()
                        }} title="Capture Photo" />
                        <Menu.Item leadingIcon="image" onPress={() => {
                            SelectImage()
            ...
            <View style={{
                ...
                <TouchableScale style={{
                    backgroundColor: themeState === 'dark' ? '#414141' : '#f0f0f0',
                    width: Dimensions.get('window').width - 50, height: 50, marginTop: 20,
                    borderRadius: 10, flexDirection: 'row', alignItems: 'center',
                    justifyContent: 'space-between', paddingHorizontal: 25,
                }} onPress={() => {
                    setDialogVisible(true)
                    setDialogMessage("Are you sure you want to De-register from Casham?")
                }}>
                    <MaterialIcons name="delete" color={Colors.red} size={25} />
                    <Text style={{
                        flex: 0.92,
                        fontSize: 15, color: Colors.red
                    }}>De-Register from Casham</Text>
                </TouchableScale>
                <TouchableScale style={{
                    backgroundColor: themeState === 'dark' ? '#414141' : '#f0f0f0',
                    width: Dimensions.get('window').width - 50, height: 50, marginTop: 20,
            ...
            <CustomSnackbar visible={snackbarVisible} label={snackbarMessage}
                onDismissSnackbar={() => {
                    setSnackbarVisible(false)
                    setSnackbarMessage("")
                }} title={"Alert!"} />
            <CustomDialog visible={dialogVisible} button1={dialogMessage == 'Are you sure you want to logout?' ?
                "Logout" : 'De-Register Now'
            }
                dialogMessage={dialogMessage} onDismiss={() => {
                    setDialogMessage("")
                    setDialogVisible(false)
                }} onPress={
                    () => {
                        dialogMessage == 'Are you sure you want to logout?' ?
                            handleLogout() : handleDeregister()
